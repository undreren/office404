...
function codeAgent(id: string, projectId: string, taskId: string | null, status: Agent['status']): Agent {
  return {
    id,
    name: id,
    personality: 'testy',
...
describe('conductor-steals-best-of-n-assist', () => {
  it('pulls a busy Best-of-N assist from another project when no idle agents exist', () => {
  ...
  it('reassigns a same-project Best-of-N assist to another role when conductor needs coverage', () => {
    const base = initialPlaying()
